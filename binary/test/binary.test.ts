...
const GOBI_GLOBAL_DIR = path.join(__dirname, "..", ".gobi");
if (fs.existsSync(GOBI_GLOBAL_DIR)) {
  fs.rmSync(GOBI_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(GOBI_GLOBAL_DIR);
...
describe("Test Suite", () => {
  ...
  beforeAll(async () => {
    ...
    const binaryPath = path.join(binaryDir, `gobi-binary${exe}`);
    const expectedItems = [
      `gobi-binary${exe}`,
      `esbuild${exe}`,
      "index.node",
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, GOBI_GLOBAL_DIR },
        });
  ...
  it("should create .gobi directory at the specified location with expected files", async () => {
    expect(fs.existsSync(GOBI_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(GOBI_GLOBAL_DIR, file);
  ...
  it("should add and delete a model from config.json", async () => {
    const model: SerializedGobiConfig["models"][number] = {
      title: "Test Model",
      provider: "openai",
      model: "gpt-3.5-turbo",

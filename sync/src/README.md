...
The first time, a Merkle tree of the codebase folder is constructed, ignoring any files in .gitignore or .gobiignore. Every file found will be returned as needing to be computed added to the index.
...
Several files are stored and updated on disk in the ~/.gobi/index folder to keep track of indexed files:

- `~/.gobi/index/tags/<dir>/<branch>/<provider_id>/merkle_tree` - the last computed Merkle tree of the codebase for a given tag
- `~/.gobi/index/tags/<dir>/<branch>/<provider_id>/.last_sync` - the last time the tag was synced
- The index cache contains a list of hashes that have already been computed both in general and per tag. These are always kept in sync.
  - `~/.gobi/index/.index_cache` - contains the global cache (flat file of hashes)
  - `~/.gobi/index/tags/<dir>/<branch>/<provider_id>/.index_cache` - contains the tag-specific cache (flat file of hashes)
  - `~/.gobi/index/rev_tags` - contains a mapping from hash to tags that the hash is currently indexed for. This is a directory of files, where each file is prefixed with the first 2 characters of the hash. The file is a JSON mapping from hash to list of tags.
...
- Only handles local files, so is not currently being used in situations where the Gobi server is on a different machine from the IDE or the workspace (Remote SSH, WSL, or a Gobi server being run for a team).
- Currently not using stat to check for recent changes to files, is instaed re-calculating the entire Merkle tree on every IDE reload. This is fine for now since it only takes 0.2 seconds on the Gobi codebase, but is a quick improvement we can make later.

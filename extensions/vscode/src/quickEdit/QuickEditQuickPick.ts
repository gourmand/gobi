...
export class QuickEdit {
    ...
    quickPick.onDidAccept(async () => {
      ...
      switch (label) {
        case UserPromptLabels.AcceptAll:
          vscode.commands.executeCommand("gobi.acceptDiff", path);
          break;
        case UserPromptLabels.RejectAll:
          vscode.commands.executeCommand("gobi.rejectDiff", path);
          break;
        case QuickEditInitialItemLabels.Submit:
          if (quickPick.value) {
            await vscode.commands.executeCommand("gobi.rejectDiff", path);
  ...
  private handleQuickPickAccept = ({
    ...
    // If not an initial item, it's a file selection. Allow gobid prompt editing.

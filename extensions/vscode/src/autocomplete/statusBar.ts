...
const statusBarItemText = (
  ...
  if (error) {
    return "$(alert) Gobi (config error)";
  }
  ...
  switch (status) {
    case undefined:
      if (loading) {
        text = "$(loading~spin) Gobi";
      } else {
        text = "Gobi";
      }
      break;
    case StatusBarStatus.Disabled:
      text = "$(circle-slash) Gobi";
      break;
    case StatusBarStatus.Enabled:
      text = "$(check) Gobi";
      break;
    case StatusBarStatus.Paused:
      text = "$(debug-pause) Gobi";
      break;
    default:
      text = "Gobi";
  }
...
export function setupStatusBar(
  ...
  statusBarItem.command = "gobi.openTabAutocompleteConfigMenu";
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration(GOBI_WORKSPACE_KEY)) {
      const enabled = getGobiWorkspaceConfig().get<boolean>(

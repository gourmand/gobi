...
export class VsCodeExtension {
  ...
  private consoleView: GobiConsoleWebviewViewProvider;
  private sidebar: GobiGUIWebviewViewProvider;
  ...
  private completionProvider: GobiCompletionProvider;
  ...
  private async updateNextEditState(
    ...
    const { config: gobiConfig } = await this.configHandler.loadConfig();
    const autocompleteModel = gobiConfig?.selectedModelByRole.autocomplete;
    ...
    if (
      ...
      !modelSupportsNext &&
      !isNextEditTest() &&
      process.env.GOBI_E2E_NON_NEXT_EDIT_TEST === "true"
    ) {
      vscode.window
        .showWarningMessage(
        ...
        .then((selection) => {
          if (selection === "Disable Next Edit") {
            ...
            vscode.commands.executeCommand(
              "gobi.openTabAutocompleteConfigMenu",
            );
          }
        });
    }
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.sidebar = new GobiGUIWebviewViewProvider(
      this.windowId,
    ...
    this.completionProvider = new GobiCompletionProvider(
      this.configHandler,
      this.ide,
      this.sidebar.webviewProtocol,
      usingFullFileDiff,
    ...
    // LLM Log view
    this.consoleView = new GobiConsoleWebviewViewProvider(
      this.windowId,
      this.extensionContext,
      this.core.llmLogger,
    );
    ...
    const globalRulesDir = path.join(getGobiGlobalPath(), "rules");
    ...
    context.subscriptions.push(
      vscode.workspace.registerTextDocumentContentProvider(
        VsCodeExtension.gobiVirtualDocumentScheme,
  ...
  static gobiVirtualDocumentScheme = EXTENSION_NAME;

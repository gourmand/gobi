...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by gobi
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while gobi makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "gobi.diffVisible", true);
  ...
  async streamEdit({
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by Gobi
      // Before adding this back, we need to distinguish between human and Gobi
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        void vscode.commands.executeCommand("gobi.rejectDiff");
      }

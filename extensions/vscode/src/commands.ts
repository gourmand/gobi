...
import { GobiConsoleWebviewViewProvider } from "./GobiConsoleWebviewViewProvider";
...
function waitForSidebarReady(
  sidebar: GobiGUIWebviewViewProvider,
  timeout: number,
  interval: number,
): Promise<boolean> {
...
const getCommandsMap: (
  ...
  sidebar: GobiGUIWebviewViewProvider,
  consoleView: GobiConsoleWebviewViewProvider,
  ...
  return {
    "gobi.acceptDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "gobi.rejectDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "gobi.acceptVerticalDiffBlock": (fileUri?: string, index?: number) => {
      captureCommandTelemetry("acceptVerticalDiffBlock");
      verticalDiffManager.acceptRejectVerticalDiffBlock(true, fileUri, index);
    },
    "gobi.rejectVerticalDiffBlock": (fileUri?: string, index?: number) => {
    ...
    "gobi.defaultQuickAction": async (args: QuickEditShowParams) => {
      ...
      vscode.commands.executeCommand("gobi.focusEdit", args);
    },
    "gobi.customQuickActionSendToChat": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "gobi.codebaseForceReIndex": async () => {
      core.invoke("index/forceReIndex", undefined);
    },
    "gobi.rebuildCodebaseIndex": async () => {
      core.invoke("index/forceReIndex", { shouldClearIndexes: true });
    },
    "gobi.docsIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: false });
    },
    "gobi.docsReIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: true });
    },
    "gobi.focusGobiInput": async () => {
      const isGobiInputFocused = await sidebar.webviewProtocol.request(
        "isGobiInputFocused",
        undefined,
        false,
      );
      ...
      if (isGobiInputFocused) {
        if (historyLength === 0) {
          hideGUI();
        } else {
          void sidebar.webviewProtocol?.request(
            "focusGobiInputWithNewSession",
            undefined,
            false,
          );
        }
    ...
    "gobi.focusGobiInputWithoutClear": async () => {
      const isGobiInputFocused = await sidebar.webviewProtocol.request(
        "isGobiInputFocused",
        undefined,
        false,
    ...
    "gobi.writeDocstringForCode": async () => {
    ...
    "gobi.optimizeCode": async () => {
      captureCommandTelemetry("optimizeCode");
      streamInlineEdit("optimize", "Optimize this code");
    },
    "gobi.fixGrammar": async () => {
    ...
    "gobi.shareSession": async (sessionId: string | undefined) => {
      if (!sessionId) {
    ...
    "gobi.focusGobiSessionId": async (
      ...
      void sidebar.webviewProtocol?.request("focusGobiSessionId", {
        sessionId,
      });
    },
    "gobi.applyCodeFromChat": () => {
      void sidebar.webviewProtocol.request("applyCodeFromChat", undefined);
    },
    "gobi.openConfigPage": () => {
      vscode.commands.executeCommand("gobi.navigateTo", "/config", false);
    },
    "gobi.selectFilesAsContext": async (
      firstUri: vscode.Uri,
      uris: vscode.Uri[],
    ) => {
      if (uris === undefined) {
      ...
      for (const uri of uris) {
        ...
        if (isDirectory) {
          for await (const fileUri of walkDirAsync(uri.toString(), ide, {
            source: "vscode gobi.selectFilesAsContext command",
          })) {
    ...
    "gobi.openTabAutocompleteConfigMenu": async () => {
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand("gobi.focusGobiInput");
        } else if (selectedOption === "$(screen-full) Open full screen chat") {
          vscode.commands.executeCommand("gobi.openInNewWindow");
        } else if (selectedOption === "$(gear) Open settings") {
          vscode.commands.executeCommand("gobi.navigateTo", "/config");
        }
    ...
    "gobi.navigateTo": (path: string, toggle: boolean) => {
      sidebar.webviewProtocol?.request("navigateTo", { path, toggle });
      focusGUI();
    },
    "gobi.startLocalOllama": () => {
      startLocalOllama(ide);
    },
    "gobi.startLocalLemonade": () => {
      startLocalLemonade(ide);
    },
    "gobi.installModel": async (
      modelName: string,
    ...
    "gobi.convertConfigJsonToConfigYaml": async () => {
        ...
        .then(async (selection) => {
          if (selection === "Read the docs") {
            await vscode.env.openExternal(
              vscode.Uri.parse("https://docs.gourmand.dev/yaml-migration"),
            );
          }
        });
    },
    "gobi.enterEnterpriseLicenseKey": async () => {
    ...
    "gobi.toggleNextEditEnabled": async () => {

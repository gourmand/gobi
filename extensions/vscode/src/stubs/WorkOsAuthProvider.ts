...
interface GobiAuthenticationSession extends AuthenticationSession {
  refreshToken: string;
  expiresInMs: number;
  loginNeeded: boolean;
}
...
export class WorkOsAuthProvider implements AuthenticationProvider, Disposable {
  ...
  private async storeSessions(value: GobiAuthenticationSession[]) {
  ...
  public async getSessions(
    scopes?: string[],
  ): Promise<GobiAuthenticationSession[]> {
    ...
    try {
      ...
      const value = JSON.parse(data) as GobiAuthenticationSession[];
      return value;
    } catch (e: any) {
  ...
  public async createSession(
    ...
    try {
      ...
      if (!token) {
        throw new Error(`Gobi login failure`);
      }
      ...
      const session: GobiAuthenticationSession = {
        id: uuidv4(),
  ...
  /**
   * Handle the redirect to VS Code (after sign in from Gobi)
   * @param scopes
   * @returns
   */
  private handleUri: (

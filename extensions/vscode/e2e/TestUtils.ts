...
export class TestUtils {
  public static GOBI_GLOBAL_DIR = process.env.GOBI_GLOBAL_DIR ?? "";

  public static getGlobalContextFilePath(): string {
    return path.join(TestUtils.GOBI_GLOBAL_DIR, "index", "globalContext.json");
  }
  ...
  public static async expectNoElement<T>(
    ...
    while (Date.now() - startTime < timeout) {
      try {
        ...
        // Gobi if there's an error (element not found)
      }
      await new Promise((resolve) => setTimeout(resolve, interval));
    }

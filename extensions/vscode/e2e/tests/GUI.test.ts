...
describe.skip("GUI Test", () => {
  ...
  before(async function () {
    ...
    await GUIActions.moveGobiToSidebar(VSBrowser.instance.driver);
  ...
  afterEach(async function () {
    ...
    await TestUtils.waitForSuccess(
      async () => (await GUISelectors.getGobiExtensionBadge(view)).click(),
      DEFAULT_TIMEOUT.XS,
    );
  ...
  describe("Agent with tools", () => {
    ...
    it("should render tool call", async () => {
      ...
      const statusMessage = await TestUtils.waitForSuccess(
        () => GUISelectors.getToolCallStatusMessage(view), // Defined in extensions/vscode/e2e/test-gobi/config.json's TOOL MOCK LLM that we are calling the exact search tool
        DEFAULT_TIMEOUT.SM,
      );

      expect(await statusMessage.getText()).contain("Gobi viewed the git diff");
    ...
    it("should cancel tool", async () => {
      ...
      const text = await statusMessage.getText();
      expect(text).contain("Gobi tried to view the git diff");
    }).timeout(DEFAULT_TIMEOUT.XL);
  });

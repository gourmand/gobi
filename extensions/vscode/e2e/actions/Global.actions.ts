...
export class GlobalActions {
  static defaultFolder = "e2e/test-gobi";
  ...
  static async setNextEditEnabled(enabled: boolean) {
    ...
    const gobiItem = await TestUtils.waitForSuccess(async () => {
    ...
    const text = await TestUtils.waitForSuccess(async () => {
      const itemText = await gobiItem.getText();
      if (!itemText || itemText.trim() === "") {
        // Fallback to textContent
        const textContent = await gobiItem.getAttribute("textContent");
        if (textContent && textContent.trim() !== "") {
          return textContent;
        }
    ...
    if (hasNE !== enabled) {
      await workbench.executeCommand("Gobi: Toggle Next Edit");

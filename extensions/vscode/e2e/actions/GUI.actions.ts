...
export class GUIActions {
  public static moveGobiToSidebar = async (driver: WebDriver) => {
    ...
    await GUIActions.executeFocusGobiInputShortcut(driver);
  ...
  public static switchToReactIframe = async () => {
    ...
    let gobiIFrame: WebElement | undefined = undefined;
    ...
    if (!gobiIFrame) {
      throw new Error("Could not find Gobi iframe");
    }

    await driver.switchTo().frame(gobiIFrame);
  ...
  public static toggleGui = async () => {
    return TestUtils.waitForSuccess(() =>
      new Workbench().executeCommand("gobi.focusGobiInput"),
    );
  };
  ...
  public static async executeFocusGobiInputShortcut(driver: WebDriver) {
    return driver
      .actions()

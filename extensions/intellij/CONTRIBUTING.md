# Contributing to Gobi (JetBrains extension) <!-- omit in toc -->
...
Gobi is built with JDK version 17 (as specified in [`./build.gradle.kts`](./build.gradle.kts)), which can be
...
- [File Expander](https://plugins.jetbrains.com/plugin/11940-file-expander) - allows you to easily preview archives as
  directories (like `build/distributions/gobi-*.zip`)
...
Select the `Run Gobi` task in the top right corner of the IDE and then select the "Debug" option.

> In community edition, use `Run Gobi (CE)` instead, which uses shell scripts instead of Ultimate-only node configs.
...
![run-extension-screenshot](../../media/run-gobi-intellij.png)

This should open a new instance on IntelliJ with the extension installed.

### Accessing files in the `.gobi` directory

When running the `Start Core Dev Server` task, we set the location of your Gobi directory to
`./extensions/.gobi-debug`. This is to
allow for changes to your `config.json` and other files during development, without affecting your actual configuration.

### Viewing logs

When using the `Run Gobi` task, we automatically tail both prompt logs and IDE logs.
...
dependencies - Displays all dependencies declared in root project 'gobi-intellij-extension'

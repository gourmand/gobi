...
import com.github.gourmand.gobiintellijextension.`gobi`.file.FileUtils
...
import com.github.gourmand.gobiintellijextension.utils.*
...
class IntelliJIDE(
    ...
    private val ADDITIONAL_SEARCH_IGNORE_FILETYPES = listOf(
        ...
        "*.gcno", "go.sum", "*.gitignore", "*.gitkeep", "*.gobiignore",
    ...
    private val gitService = GitService(project, gobiPluginService)
    ...
    override suspend fun getIdeInfo(): IdeInfo {
        ...
        val pluginId = GobiConstants.PLUGIN_ID
    ...
    suspend fun enableHubgourmand(): Boolean {
        return true
    }
    ...
    override suspend fun getIdeSettings(): IdeSettings {
        ...
        return IdeSettings(
            remoteConfigServerUrl = settings.gobiState.remoteConfigServerUrl,
            remoteConfigSyncPeriod = settings.gobiState.remoteConfigSyncPeriod,
            userToken = settings.gobiState.userToken ?: "",
            gobiTestEnvironment = "production",
    ...
    override suspend fun getGobiDir(): String {
        return getGobiGlobalPath()
    }
    ...
    override suspend fun showDiff(filepath: String, newContents: String, stepIndex: Int) {
        gobiPluginService.diffManager?.showDiff(filepath, newContents, stepIndex)
    }
    ...
    override suspend fun getFileResults(pattern: String, maxResults: Int?): List<String> {
        ...
        if (ideInfo.remoteName == "local") {
            try {
                ...
                service<GobiSentryService>().report(exception, message)
    ...
    override suspend fun showToast(type: ToastType, message: String, vararg otherParams: Any): Any {
        return withContext(Dispatchers.Default) {
            ...
            val notification = NotificationGroupManager.getInstance().getNotificationGroup("Gobi")
                .createNotification(message, notificationType).setIcon(Icons.Gobi)
    ...
    private fun workspaceDirectories(): Array<String> {
        val dirs = this.gobiPluginService.workspacePaths

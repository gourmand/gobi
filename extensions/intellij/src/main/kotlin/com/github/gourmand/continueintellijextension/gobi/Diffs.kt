...
import com.github.gourmand.gobiintellijextension.utils.getAltKeyLabel
...
fun getDiffDirectory(): File {
    ...
    val diffDirPath = Paths.get(homeDirectory).resolve(".gobi").resolve(".diffs").toString()
...
class DiffManager(private val project: Project) : DumbAware {
    ...
    fun acceptDiff(file2: String?) {
        ...
        project.service<GobiPluginService>().ideProtocolClient?.sendAcceptRejectDiff(true, diffInfo.stepIndex)
    ...
    fun rejectDiff(file2: String?) {
        ...
        gobiPluginService.ideProtocolClient?.deleteAtIndex(diffInfo.stepIndex)
        gobiPluginService.ideProtocolClient?.sendAcceptRejectDiff(false, diffInfo.stepIndex)
        gobiPluginService.coreMessenger?.request("cancelApply", null, null) {}
        cleanUpFile(file)
    }
    ...
    private fun openDiffWindow(
        ...
        val diffRequest = SimpleDiffRequest("Gobi Diff", content1, content2, "Old", "New")

package com.github.gourmand.gobiintellijextension.auth
...
@Service
class GobiAuthService {
    private val coroutineScope = CoroutineScope(Dispatchers.IO)
    private val log = Logger.getInstance(GobiAuthService::class.java)

    companion object {
        private const val CREDENTIALS_USER = "GobiAuthUser"
        private const val ACCESS_TOKEN_KEY = "GobiAccessToken"
        private const val REFRESH_TOKEN_KEY = "GobiRefreshToken"
        private const val ACCOUNT_ID_KEY = "GobiAccountId"
        private const val ACCOUNT_LABEL_KEY = "GobiAccountLabel"
    }

    private fun getControlPlaneUrl(): String {
        val env = service<GobiExtensionSettings>().gobiState.gobiTestEnvironment;
        when (env) {
            "none" -> return "https://control-plane-api-service-i3dqylpbqa-uc.a.run.app"
            "local" -> return "http://localhost:3001"
            "production" -> return "https://api.gourmand.dev"
            "test" -> return "https://api-test.gourmand.dev"
        }
    ...
    fun startAuthFlow(project: Project, useOnboarding: Boolean) {
        ...
        ApplicationManager.getApplication().invokeLater {
            val dialog = GobiAuthDialog(useOnboarding, url) { token ->
                // Store the token
    ...
    private fun updateRefreshToken(token: String) {
        try {
            ...
            service<GobiSentryService>().report(e, "Exception while refreshing token ${e.message}")
        }
    }

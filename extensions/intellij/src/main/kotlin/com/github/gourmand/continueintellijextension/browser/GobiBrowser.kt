...
class GobiBrowser(private val project: Project): Disposable {

    private val log = Logger.getInstance(GobiBrowser::class.java.simpleName)
    ...
    init {
        CefApp.getInstance().registerSchemeHandlerFactory("http", "gobi", CustomSchemeHandlerFactory())
        ...
        myJSQueryOpenInBrowser.addHandler { msg: String? ->
            ...
            if (MessageTypes.PASS_THROUGH_TO_CORE.contains(messageType)) {
                project.service<GobiPluginService>().coreMessenger?.request(messageType, data, messageId) { data ->
            ...
            if (msg != null) {
                project.service<GobiPluginService>().ideProtocolClient?.handleMessage(msg) { data ->
                    sendToWebview(
                        messageType,
                        mapOf(
                            "status" to "success",
        ...
        project.service<GobiPluginService>().onProtocolClientInitialized {
    ...
    private companion object {
        ...
        private fun getGuiUrl() =
            System.getenv("GUI_URL") ?: "http://gobi/index.html"

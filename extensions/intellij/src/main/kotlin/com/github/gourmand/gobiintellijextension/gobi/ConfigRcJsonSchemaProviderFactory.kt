...
import com.github.gourmand.gobiintellijextension.activities.GobiPluginStartupActivity
...
class ConfigRcJsonSchemaFileProvider : JsonSchemaFileProvider {
    ...
    override fun getSchemaFile(): VirtualFile? {
        GobiPluginStartupActivity::class.java.getClassLoader().getResourceAsStream("gobi_rc_schema.json")
            .use { `is` ->
                if (`is` == null) {
                    throw IOException("Resource not found: gobi_rc_schema.json")
                }
                val content = StreamUtil.readText(`is`, StandardCharsets.UTF_8)
                val filepath = Paths.get(getGobiGlobalPath(), "gobi_rc_schema.json").toString()

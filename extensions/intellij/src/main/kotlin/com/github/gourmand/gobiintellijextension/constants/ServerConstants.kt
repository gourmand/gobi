 ...
 */
package com.github.gourmand.gobiintellijextension.constants
...
fun getGobiGlobalPath(): String {
  val gobiPath = Paths.get(System.getProperty("user.home"), ".gobi")
  if (Files.notExists(gobiPath)) {
    Files.createDirectories(gobiPath)
  }
  return gobiPath.toString()
}

fun getGobiRemoteConfigPath(remoteHostname: String): String {
  val path = Paths.get(getGobiGlobalPath(), ".configs")
...
fun getConfigJsonPath(remoteHostname: String? = null): String {
  val path =
      Paths.get(
          if (remoteHostname != null) getGobiRemoteConfigPath(remoteHostname)
          else getGobiGlobalPath(),
          "config.json")
  if (Files.notExists(path)) {
...
fun getSessionsDir(): String {
  val path = Paths.get(getGobiGlobalPath(), "sessions")
  if (Files.notExists(path)) {
    Files.createDirectories(path)
  }
  return path.toString()

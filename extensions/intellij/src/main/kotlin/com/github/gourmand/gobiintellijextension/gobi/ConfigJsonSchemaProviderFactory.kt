...
class ConfigJsonSchemaFileProvider : JsonSchemaFileProvider {
    override fun isAvailable(file: VirtualFile): Boolean {
        return file.path.endsWith("/.gobi/config.json") || file.path.endsWith("\\.gobi\\config.json")
    }
    ...
    override fun getSchemaFile(): VirtualFile? {
        GobiPluginStartupActivity::class.java.getClassLoader().getResourceAsStream("config_schema.json")
            .use { `is` ->
                if (`is` == null) {
                    throw IOException("Resource not found: config_schema.json")
                }
                val content = StreamUtil.readText(`is`, StandardCharsets.UTF_8)
                val filepath = Paths.get(getGobiGlobalPath(), "config_schema.json").toString()

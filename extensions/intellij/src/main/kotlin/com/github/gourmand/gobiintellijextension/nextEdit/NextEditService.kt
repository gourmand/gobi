package com.github.gourmand.gobiintellijextension.nextEdit
...
class GobiNextEditService(private val project: Project) : NextEditService {
    private val displayedCompletions = mutableMapOf<String, NextEditOutcome>()

    private val coreMessenger: CoreMessenger?
        get() = project.service<GobiPluginService>().coreMessenger
...
//                val nextEditItem = processedItem?.let { convertToNextEditItem(it) }
    ...
    private fun parseProcessedItem(response: Any?): ProcessedItem? {
        return try {
            ...
            val range = com.github.gourmand.gobiintellijextension.Range(startPosition, endPosition)
            val location = com.github.gourmand.gobiintellijextension.RangeInFile(filepath, range)

...
import com.github.gourmand.gobiintellijextension.error.GobiTelemetryStatus
import com.posthog.java.PostHog
import io.mockk.mockk
import io.mockk.verify
import junit.framework.TestCase

class GobiPostHogServiceTest : TestCase() {

    fun `test report when telemetry is enabled`() {
        val postHog = mockk<PostHog>(relaxed = true)
        val service = GobiPostHogService(TestTelemetryService(true), postHog)
    ...
    fun `test don't report when telemetry is disabled`() {
        val postHog = mockk<PostHog>(relaxed = true)
        val service = GobiPostHogService(TestTelemetryService(false), postHog)
        service.capture("test", mapOf("test" to "test"))

...
export async function compactChatHistory(
  ...
  const compactionPrompt: ChatHistoryItem = {
    message: {
      role: "user" as const,
      content:
        "Please provide a concise summary of our conversation so far, capturing the key context, decisions made, and current state. Format this as a single comprehensive message that preserves all important information needed to gobi our work. You do not need to recap the system message, as this will remain. Make sure it is clear what the current stream of work was at the very end prior to compaction so that you can gobi exactly where you left off without missing any information.",
    },
    contextItems: [],
  };

...
import { GOBI_ASCII_ART, getDisplayableAsciiArt } from "./asciiArt.js";
...
describe("asciiArt", () => {
  ...
  describe("getDisplayableAsciiArt", () => {
    it("should return full ASCII art when terminal is wide enough", () => {
      ...
      expect(result).toBe(GOBI_ASCII_ART);
    });
    ...
    it("should return CN ASCII art version when terminal is too narrow", () => {
      ...
      expect(result).not.toBe(GOBI_ASCII_ART);
      // Should be much shorter than the full ASCII art
      expect(result.length).toBeLessThan(GOBI_ASCII_ART.length / 2);
    });

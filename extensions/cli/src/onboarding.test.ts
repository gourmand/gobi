...
describe("onboarding config flag handling", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "gobi-test-"));
...
`,
  ...
  test("should not fall back to default config when explicit config fails", async () => {
    ...
    try {
      ...
      expect(message).not.toContain("~/.gobi/config.yaml");
...
describe("GOBI_USE_BEDROCK environment variable", () => {
  const mockConsoleLog = vi.fn();
  let mockAuthConfig: AuthConfig;
  const originalEnv = process.env.GOBI_USE_BEDROCK;
  ...
  afterEach(() => {
    if (originalEnv) {
      process.env.GOBI_USE_BEDROCK = originalEnv;
    } else {
      delete process.env.GOBI_USE_BEDROCK;
    }
    vi.restoreAllMocks();
    vi.doUnmock("./config.js");
  });

  test("should bypass interactive options when GOBI_USE_BEDROCK=1", async () => {
    process.env.GOBI_USE_BEDROCK = "1";
    ...
    expect(mockConsoleLog).toHaveBeenCalledWith(
      expect.stringContaining(
        "✓ Using AWS Bedrock (GOBI_USE_BEDROCK detected)",
      ),
    );
  });

  test("should not bypass when GOBI_USE_BEDROCK is not '1'", async () => {
    process.env.GOBI_USE_BEDROCK = "0";

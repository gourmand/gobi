...
describe("E2E: Resume Flag", () => {
  ...
  it("should resume from previous session when using --resume flag", async () => {
    ...
    const firstResult = await runCLI(context, {
      ...
      env: {
        ...
        GOBI_CLI_TEST_SESSION_ID: "test-session-123",
        GOBI_GLOBAL_DIR: path.join(context.testDir, ".gobi"),
      },
      timeout: 15000,
    });
    ...
    const sessionDir = path.join(context.testDir, ".gobi", "sessions");

    // Ensure session directory exists
    try {
      await fs.mkdir(sessionDir, { recursive: true });
    } catch (error) {
      // Directory already exists, gobi
    }
  ...
  it("should handle --resume when no previous session exists", async () => {
    ...
    const result = await runCLI(context, {
      args: ["--resume", "--config", context.configPath],
      env: {
        GOBI_CLI_TEST_SESSION_ID: "no-session-456",
        GOBI_GLOBAL_DIR: path.join(context.testDir, ".gobi"),
      },
      timeout: 15000,
    });
  ...
  it("should resume and allow new messages to be added", async () => {
    ...
    const firstResult = await runCLI(context, {
      ...
      env: {
        GOBI_CLI_TEST_SESSION_ID: sessionId,
        GOBI_GLOBAL_DIR: path.join(context.testDir, ".gobi"),
      },
      timeout: 15000,
    });

...
function getAuthConfigPath() {
  ...
  return path.join(gobiHome, "auth.json");
}
...
export function loadAuthConfig(): AuthConfig {
  // If GOBI_API_KEY environment variable exists, use that instead
  if (process.env.GOBI_API_KEY) {
    return {
      accessToken: process.env.GOBI_API_KEY,
      organizationId: null,
    };
  }
...
export function saveAuthConfig(config: AuthenticatedConfig): void {
  // If using GOBI_API_KEY environment variable, don't save anything
  if (process.env.GOBI_API_KEY) {
    return;
  }
...
export async function login(): Promise<AuthConfig> {
  // If GOBI_API_KEY environment variable exists, use that instead
  if (process.env.GOBI_API_KEY) {
    console.info(chalk.green("Using GOBI_API_KEY from environment variables"));
    return {
      accessToken: process.env.GOBI_API_KEY,
      organizationId: null,
    };
...
> {
  ...
  // If using GOBI_API_KEY environment variable, organization switching is not supported
  if (isEnvironmentAuthConfig(authConfig)) {
    return null;
  }
...
export function logout(): void {
  const gobiHome =
    process.env.GOBI_GLOBAL_DIR || path.join(os.homedir(), ".gobi");
  const onboardingFlagPath = path.join(gobiHome, ".onboarding_complete");
  ...
  if (process.env.GOBI_API_KEY) {
    console.info(
      chalk.yellow(
        "Using GOBI_API_KEY from environment variables, nothing to log out",
      ),
    );
    return;
  }

...
describe("loadMcpFromHub", () => {
  ...
  it("should handle HTTP errors", async () => {
    ...
    await expect(loadMcpFromHub("gourmand/nonexistent-mcp")).rejects.toThrow(
      'Failed to load mcp from hub "gourmand/nonexistent-mcp": HTTP 404: Not Found',
    );
  });
  ...
  it("should handle network errors", async () => {
    ...
    await expect(loadMcpFromHub("gourmand/test-mcp")).rejects.toThrow(
      'Failed to load mcp from hub "gourmand/test-mcp": Network error',
    );
  });
  ...
  it("should parse JSON configuration files", async () => {
    ...
    const result = await loadMcpFromHub("gourmand/test-mcp");
  ...
  it("should handle missing configuration files", async () => {
    ...
    const result = await loadMcpFromHub("gourmand/no-config");
  ...
  it("should handle invalid JSON configuration", async () => {
    ...
    await expect(loadMcpFromHub("gourmand/invalid-json")).rejects.toThrow(
      'Failed to load mcp from hub "gourmand/invalid-json": Invalid JSON',
    );
  });
});

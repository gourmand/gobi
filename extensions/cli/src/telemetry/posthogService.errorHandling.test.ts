...
describe("PosthogService Error Handling", () => {
  ...
  it("should not capture when service is disabled", async () => {
    ...
    const originalEnv = process.env.GOBI_ALLOW_ANONYMOUS_TELEMETRY;
    ...
    if (originalEnv === undefined) {
      delete process.env.GOBI_ALLOW_ANONYMOUS_TELEMETRY;
    } else {
      process.env.GOBI_ALLOW_ANONYMOUS_TELEMETRY = originalEnv;
    }
  });
});

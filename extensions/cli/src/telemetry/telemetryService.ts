...
import { isGitHubActions, isGobiRemoteAgent } from "../util/git.js";
...
class TelemetryService {
  ...
  private loadConfig(): TelemetryConfig {
    ...
    const enabled =
      process.env.GOBI_CLI_ENABLE_TELEMETRY !== "0" && hasOtelConfig;
  ...
  private initialize() {
    try {
      ...
      const resource = resourceFromAttributes({
        [SEMRESATTRS_SERVICE_NAME]: "gobi-cli",
      ...
      this.meter = metrics.getMeter("gobi-cli", getVersion());
  ...
  private initializeMetrics() {
    ...
    this.sessionCounter = this.meter.createCounter("gobi_cli_session_count", {
    ...
    this.commitCounter = this.meter.createCounter("gobi_cli_commit_count", {
      description: "Number of git commits created",
      unit: "count",
    });

    this.costCounter = this.meter.createCounter("gobi_cli_cost_usage", {
      description: "Cost of the Gobi CLI session",
      unit: "USD",
    });

    this.tokenCounter = this.meter.createCounter("gobi_cli_token_usage", {
    ...
    // Additional Gobi CLI specific metrics
  ...
  public recordSessionStart() {
    ...
    const sessionAttributes = this.getStandardAttributes({
      ...
      is_gobi_remote_agent: isGobiRemoteAgent().toString(),
    });

...
describe("processPromptOrRule (loadRuleFromHub integration)", () => {
  ...
  describe("loadRuleFromHub", () => {
    it("should successfully load a rule from the Gobi Hub", async () => {
      ...
      const result = await processPromptOrRule("gourmand/sentry-nextjs");

      expect(result).toBe(ruleContent);
      expect(mockFetch).toHaveBeenCalledWith(
        new URL(
          "v0/gourmand/sentry-nextjs/latest/download",
          "https://api.gourmand.dev/",
        ),
        { headers: {} },
      );
    });
    ...
    it("should handle HTTP errors when loading from hub", async () => {
      ...
      await expect(
        processPromptOrRule("gourmand/nonexistent-rule"),
      ).rejects.toThrow(
        'Failed to load rule from hub "gourmand/nonexistent-rule": HTTP 404: Not Found',
      );
    });
    ...
    it("should handle network errors when loading from hub", async () => {
      ...
      await expect(
        processPromptOrRule("gourmand/sentry-nextjs"),
      ).rejects.toThrow(
        'Failed to load rule from hub "gourmand/sentry-nextjs": Network error',
      );
    });
    ...
    it("should handle empty zip file", async () => {
      ...
      await expect(processPromptOrRule("gourmand/empty-rule")).rejects.toThrow(
        'Failed to load rule from hub "gourmand/empty-rule": No rule content found in downloaded zip file',
      );
    });
    ...
    it("should handle zip with only directories", async () => {
      ...
      await expect(
        processPromptOrRule("gourmand/directory-only"),
      ).rejects.toThrow(
        'Failed to load rule from hub "gourmand/directory-only": No rule content found in downloaded zip file',
      );
    });
    ...
    it("should prefer markdown files and ignore other file types", async () => {
      ...
      const result = await processPromptOrRule("gourmand/mixed-files");
    ...
    it("should handle multiple markdown files and use the first one", async () => {
      ...
      const result = await processPromptOrRule("gourmand/multiple-rules");
    ...
    it("should handle malformed zip files", async () => {
      ...
      await expect(
        processPromptOrRule("gourmand/corrupted-zip"),
      ).rejects.toThrow(
        'Failed to load rule from hub "gourmand/corrupted-zip"',
      );
    });
  });

...
function determineConfigSource(
  ...
  if (authConfig === null) {
    ...
    const defaultConfigPath = path.join(env.gobiHome, "config.yaml");
...
/**
 * Loads default config.yaml from ~/.gobi/config.yaml
 */
...
/**
 * Loads the default gourmand/default-agent
 */
...
/**
 * Loads an assistant by slug from the Gobi platform
 */
...
function getUriFromSource(source: ConfigSource): string | null {
  switch (source.type) {
    ...
    case "saved-uri":
      return source.uri;
    case "default-config-yaml":
      return `file://${path.join(env.gobiHome, "config.yaml")}`;
    default:
      return null;
  }
}

...
import { DefaultApiInterface } from "@gourmanddev/sdk/dist/api";
...
export class CLIPlatformClient implements PlatformClient {
  ...
  private findSecretInLocalEnvFiles(fqsn: FQSN): SecretResult | undefined {
    ...
    // Then check in priority order: ~/.gobi/.env, <workspace>/.gobi/.env, <workspace>/.env
    const workspaceDir = process.cwd();
    const envPaths = [
      path.join(env.gobiHome, ".env"),
      path.join(workspaceDir, ".gobi", ".env"),

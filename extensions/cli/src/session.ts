...
function getSessionDir(): string {
  ...
  if (process.env.GOBI_CLI_TEST && process.env.HOME) {
    const sessionDir = path.join(process.env.HOME, ".gobi", "sessions");
  ...
  // Use GOBI_GLOBAL_DIR if set (for testing)
  const gobiHome =
    process.env.GOBI_GLOBAL_DIR || path.join(os.homedir(), ".gobi");
  const sessionDir = path.join(gobiHome, "sessions");
...
class SessionManager {
  ...
  getCurrentSession(): Session {
    if (!this.currentSession) {
      ...
      const sessionId = process.env.GOBI_CLI_TEST_SESSION_ID
        ? process.env.GOBI_CLI_TEST_SESSION_ID
        : uuidv4();
...
export async function getRemoteSessions(): Promise<ExtendedSessionMetadata[]> {
  try {
    ...
    if (
      ...
      !authConfig.userEmail.endsWith("@gourmand.dev")
    ) {
      return [];
    }

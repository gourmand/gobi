...
const CONFIG_PATH = path.join(env.gobiHome, "config.yaml");
  ...
  const handleOptionSelect = async () => {
    if (selectedOption === 1) {
      ...
      try {
        ...
        setSuccessMessage(
          `Browser opened to ${modelsUrl}. After setting up your models subscription, press Enter to gobi.`,
        );
        setCurrentStep("success");
      } catch {
        setErrorMessage(
          `Could not open browser automatically. Please visit: ${modelsUrl}. After setting up your models subscription, press Enter to gobi.`,
        );
  ...
  if (currentStep === "choice") {
    ...
    return (
      <Box {...defaultBoxStyles("yellow")}>
        ...
        <Text>Choose how you'd like to gobi:</Text>
        <Text></Text>
        {options.map((option) => (
          <Text
            key={option.num}
  ...
  if (currentStep === "success") {
    return (
      <Box {...defaultBoxStyles("green")}>
        ...
        <Text color="gray">
          {wasModelsSetup
            ? "Press Enter to reload and gobi with your new subscription"
            : "Press Enter to gobi your conversation"}
        </Text>
      </Box>
    );
  }

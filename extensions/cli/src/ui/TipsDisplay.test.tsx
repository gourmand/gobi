...
import { GOBI_CLI_TIPS, TipsDisplay, shouldShowTip } from "./TipsDisplay.js";
...
describe("TipsDisplay", () => {
  ...
  it("renders a random tip from the tips array", () => {
    ...
    const expectedTipIndex = Math.floor(0.5 * GOBI_CLI_TIPS.length);
    const expectedTip = GOBI_CLI_TIPS[expectedTipIndex];
  ...
  it("always renders the tip structure correctly", () => {
    ...
    const containsAtLeastOneTip = GOBI_CLI_TIPS.some(
...
describe("GOBI_CLI_TIPS", () => {
  it("contains at least 6 tips", () => {
    expect(GOBI_CLI_TIPS.length).toBeGreaterThanOrEqual(6);
  });

  it("all tips are non-empty strings", () => {
    GOBI_CLI_TIPS.forEach((tip, index) => {
  ...
  it("tips are helpful and informative", () => {
    ...
    const allTipsText = GOBI_CLI_TIPS.join(" ").toLowerCase();

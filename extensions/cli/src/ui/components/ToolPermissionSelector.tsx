...
const getPermissionOptions = (): PermissionOption[] => {
  return [
    { id: "approve", name: "Gobi", color: "green", approved: true },
    {
      id: "approve_policy",
      name: "Gobi + don't ask again",
      color: "cyan",
      approved: true,
      createPolicy: true,
    },
    {
      id: "deny_stop",
      name: "No, and tell Gobi what to do differently",
      color: "yellow",
      approved: false,
      stopStream: true,
    },
...
export const ToolPermissionSelector: React.FC<ToolPermissionSelectorProps> = ({
  ...
  useInput((input, key) => {
    ...
    // Shift+Tab to gobi with policy creation
    if (key.tab && key.shift) {
  ...
  return (
    <Box {...defaultBoxStyles("magenta")}>
      ...
      <Box marginTop={1} flexDirection="column">
        <Text color="dim">Would you like to gobi?</Text>
        {showDynamicWarning && (
          <Box marginTop={1}>

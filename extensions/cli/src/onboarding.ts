...
export async function runOnboardingFlow(
  ...
  // Step 2: Check for GOBI_USE_BEDROCK environment variable first (before test env check)
  if (process.env.GOBI_USE_BEDROCK === "1") {
    console.log(chalk.blue("✓ Using AWS Bedrock (GOBI_USE_BEDROCK detected)"));
    return true;
  }
  ...
  console.log(chalk.white("1. ⏩ Log in with Gobi"));
...
export async function isFirstTime(): Promise<boolean> {
  return !fs.existsSync(path.join(env.gobiHome, ".onboarding_complete"));
}

export async function markOnboardingComplete(): Promise<void> {
  const flagPath = path.join(env.gobiHome, ".onboarding_complete");

...
export class UpdateService extends BaseService<UpdateServiceState> {
  ...
  private async checkAndAutoUpdate() {
    ...
    try {
      ...
      if (
        ...
        !process.env.GOBI_CLI_AUTO_UPDATED //Already auto updated, preventing sequential auto-update
      ) {
        ...
        this.setState({
          ...
          message: isUpdateAvailable
            ...
            : `Gobi CLI v${this.currentState.currentVersion}`,
          isUpdateAvailable,
          latestVersion,
        });
      }
      ...
      this.setState({
        ...
        message: `Gobi CLI v${this.currentState.currentVersion}`,
        error,
      });
    }
  }
  ...
  async performUpdate(isAutoUpdate?: boolean) {
    ...
    try {
      ...
      // Install the update
      const { stdout, stderr } = await execAsync("npm i -g @gourmanddev/cli");

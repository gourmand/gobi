...
describe("Model Persistence for Unauthenticated Users", () => {
  let testDir: string;
  let originalGobiHome: string | undefined;
  ...
  beforeEach(() => {
    ...
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), "gobi-test-"));
    originalGobiHome = process.env.GOBI_GLOBAL_DIR;
    process.env.GOBI_GLOBAL_DIR = testDir;
  ...
  afterEach(() => {
    ...
    if (originalGobiHome) {
      process.env.GOBI_GLOBAL_DIR = originalGobiHome;
    } else {
      delete process.env.GOBI_GLOBAL_DIR;
    }
  });
  ...
  test("should not save to GlobalContext when using GOBI_API_KEY", () => {
    // Clear any existing model
    updateModelName(null);
    expect(getPersistedModelName()).toBeNull();

    // Capture original value to restore after test
    const originalApiKey = process.env.GOBI_API_KEY;
    process.env.GOBI_API_KEY = "test-api-key";
    ...
    if (originalApiKey === undefined) {
      delete process.env.GOBI_API_KEY;
    } else {
      process.env.GOBI_API_KEY = originalApiKey;
    }
  });

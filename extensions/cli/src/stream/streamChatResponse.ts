...
function processChunk(options: ProcessChunkOptions): {
  ...
  if (!chunk.choices || !chunk.choices[0]) {
    return { aiResponse, shouldGobi: true };
  }
  ...
  return { aiResponse: updatedResponse, shouldGobi: true };
}
...
// Process a single streaming response and return whether we need to gobi
export async function processStreamingResponse(
  ...
  return {
    ...
    shouldGobi: validToolCalls.length > 0,
  };
}

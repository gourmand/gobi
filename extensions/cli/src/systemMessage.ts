...
function getDirectoryStructure(): string {
  try {
    const walker = new WalkerSync({
      ...
      includeEmpty: false,
      follow: false,
      ignoreFiles: [".gitignore", ".gobiignore", ".customignore"],
    });
...
const baseSystemMessage = `You are an agent in the Gobi CLI. Given the user's prompt, you should use the tools available to you to answer the user's question.
...
export async function constructSystemMessage(
  ...
  try {
    ...
    // If there's any error reading the file, gobi without agent content
...
Generated with [Gobi](https://gourmand.dev)

Co-Authored-By: Gobi <noreply@gourmand.dev>
</context>\n`;
  }

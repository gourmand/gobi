...
import {
  ...
  anonymizeUserInfo,
} from "@gourmanddev/core/util/sentry/anonymization";
import { SENTRY_DSN } from "@gourmanddev/core/util/sentry/constants";
import posthog from "posthog-js";
import { PostHogProvider } from "posthog-js/react";
import { PropsWithChildren, useEffect } from "react";
import { useAuth } from "../context/Auth";
import { useAppSelector } from "../redux/hooks";
import { isPrerelease } from "../util";
import { isGobiTeamMember } from "../util/isGobiTeamMember";
...
const TelemetryProviders = ({ children }: PropsWithChildren) => {
  ...
  const hasGobiEmail = isGobiTeamMember(
  ...
  useEffect(() => {
    ...
    // Sentry depends on both allowAnonymousTelemetry and hasGobiEmail
    if (allowAnonymousTelemetry && hasGobiEmail) {
  ...
  }, [allowAnonymousTelemetry, hasGobiEmail]);

  // Conditionally wrap with ErrorBoundary when telemetry is enabled and user is Gobi team member
  const content =
    allowAnonymousTelemetry && hasGobiEmail ? (

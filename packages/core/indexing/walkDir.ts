...
import { getGlobalGobiIgArray } from "./gobiignore";
...
export async function getIgnoreContext(
  ...
  // These are done separately so that .gobiignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const gobiIgnoreFile = dirFiles.find((name) => name === ".gobiignore");
  ...
  const getGobiIgnorePatterns = async () => {
    if (gobiIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/.gobiignore`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .gobiignore - this is combined for speed
    .add(ignoreArrays[1]); // local .gobiignore

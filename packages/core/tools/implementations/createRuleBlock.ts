import { createRuleMarkdown } from "@gourmanddev/config-yaml";
import { ToolImpl } from "./index";
import { RuleWithSource } from "../../index";
...
export const createRuleBlockImpl: ToolImpl = async (args, extras) => {
  ...
  const [localGobiDir] = await extras.ide.getWorkspaceDirs();
  const ruleFilePath = createRuleFilePath(localGobiDir, name);

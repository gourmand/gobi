...
const GOBI_PUBLIC_KEY = `-----BEGIN PUBLIC KEY-----
...
export function validateLicenseKey(licenseKey: string): {
  ...
  try {
    ...
    const isValid = verify.verify(GOBI_PUBLIC_KEY, signature, "base64");
...
const MACOS_MDM_PATHS = [
  ...
  "/Library/Managed Preferences/dev.gobi.app.plist",
  // User-specific MDM plist
  path.join(os.homedir(), "Library/Managed Preferences/dev.gobi.app.plist"),
];
...
function readMdmKeysWindows(): MdmKeys | undefined {
  try {
    ...
    const regPath = "HKLM\\Software\\Gobi\\MDM";
    const userRegPath = "HKCU\\Software\\Gobi\\MDM";
...
const LINUX_MDM_PATHS = [
  ...
  "/etc/gobi/mdm.json",
  "/var/lib/gobi/mdm.json",
  // User-specific configuration
  path.join(os.homedir(), ".config/gobi/mdm.json"),
];
...
function writeMdmKeysLinux(licenseKey: string): boolean {
  try {
    ...
    const userMdmPath = path.join(os.homedir(), ".config/gobi/mdm.json");

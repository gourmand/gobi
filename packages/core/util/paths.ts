...
import { ConfigYaml, DevEventName } from "@gourmanddev/config-yaml";
import * as JSONC from "comment-json";
import dotenv from "dotenv";

import { IdeType, SerializedGobiConfig } from "../index";
import { defaultConfig } from "../config/default";
import Types from "../config/types";

dotenv.config();

const GOBI_GLOBAL_DIR = (() => {
  const configPath = process.env.GOBI_GLOBAL_DIR;
  if (configPath) {
  ...
  return path.join(os.homedir(), ".gobi");
})();
...
export function getChromiumPath(): string {
  return path.join(getGobiUtilsPath(), ".chromium-browser-snapshots");
}

export function getGobiUtilsPath(): string {
  const utilsPath = path.join(getGobiGlobalPath(), ".utils");
...
export function getGlobalGobiIgnorePath(): string {
  const gobiIgnorePath = path.join(getGobiGlobalPath(), ".gobiignore");
  if (!fs.existsSync(gobiIgnorePath)) {
    fs.writeFileSync(gobiIgnorePath, "");
  }
  return gobiIgnorePath;
}

export function getGobiGlobalPath(): string {
  // This is ~/.gobi on mac/linux
  const gobiPath = GOBI_GLOBAL_DIR;
  if (!fs.existsSync(gobiPath)) {
    fs.mkdirSync(gobiPath);
  }
  return gobiPath;
}

export function getSessionsFolderPath(): string {
  const sessionsPath = path.join(getGobiGlobalPath(), "sessions");
...
export function getIndexFolderPath(): string {
  const indexPath = path.join(getGobiGlobalPath(), "index");
...
export function getSharedConfigFilePath(): string {
  return path.join(getGobiGlobalPath(), "sharedConfig.json");
}
...
export function getConfigJsonPath(): string {
  const p = path.join(getGobiGlobalPath(), "config.json");
  return p;
}

export function getConfigYamlPath(ideType?: IdeType): string {
  const p = path.join(getGobiGlobalPath(), "config.yaml");
  if (!fs.existsSync(p) && !fs.existsSync(getConfigJsonPath())) {
    if (ideType === "jetbrains") {
      // https://github.com/gourmand/gobi/pull/7224
...
export function getConfigTsPath(): string {
  const p = path.join(getGobiGlobalPath(), "config.ts");
  if (!fs.existsSync(p)) {
    fs.writeFileSync(p, DEFAULT_CONFIG_TS_CONTENTS);
  }

  const typesPath = path.join(getGobiGlobalPath(), "types");
  ...
  const packageJsonPath = path.join(getGobiGlobalPath(), "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "gobi-config",
        version: "1.0.0",
        description: "My Gobi Configuration",
        main: "config.js",
      }),
    );
  }
...
export function getConfigJsPath(): string {
  ...
  return path.join(getGobiGlobalPath(), "out", "config.js");
}

export function getTsConfigPath(): string {
  const tsConfigPath = path.join(getGobiGlobalPath(), "tsconfig.json");
...
export function getGobiRcPath(): string {
  // Disable indexing of the config folder to prevent infinite loops
  const gobircPath = path.join(getGobiGlobalPath(), ".gobirc.json");
  if (!fs.existsSync(gobircPath)) {
    fs.writeFileSync(
      gobircPath,
      JSON.stringify(
        {
...
function getDevDataPath(): string {
  const sPath = path.join(getGobiGlobalPath(), "dev_data");
...
function editConfigJson(
  callback: (config: SerializedGobiConfig) => SerializedGobiConfig,
): void {
...
export function editConfigFile(
  configJsonCallback: (config: SerializedGobiConfig) => SerializedGobiConfig,
...
function getMigrationsFolderPath(): string {
  const migrationsPath = path.join(getGobiGlobalPath(), ".migrations");
...
export function getRemoteConfigsFolderPath(): string {
  const dir = path.join(getGobiGlobalPath(), ".configs");
...
export function getGobiDotEnv(): { [key: string]: string } {
  const filepath = path.join(getGobiGlobalPath(), ".env");
  if (fs.existsSync(filepath)) {
...
export function getLogsDirPath(): string {
  const logsPath = path.join(getGobiGlobalPath(), "logs");
...
export function getGlobalFolderWithName(name: string): string {
  return path.join(getGobiGlobalPath(), name);
}
...
export function getRepoMapFilePath(): string {
  return path.join(getGobiUtilsPath(), "repo_map.txt");
}

export function getEsbuildBinaryPath(): string {
  return path.join(getGobiUtilsPath(), "esbuild");
}
...
export function getLocalEnvironmentDotFilePath(): string {
  return path.join(getGobiGlobalPath(), ".local");
}

export function getStagingEnvironmentDotFilePath(): string {
  return path.join(getGobiGlobalPath(), ".staging");
}

export function getDiffsDirectoryPath(): string {
  const diffsPath = path.join(getGobiGlobalPath(), ".diffs"); // .replace(/^C:/, "c:"); ??

import { BlockType, RULE_FILE_EXTENSION } from "@gourmanddev/config-yaml";
...
describe("getFileContent", () => {
  ...
  test("generates correct content for different block types", () => {
    ...
    expect(docsResult).toContain("startUrl: https://docs.gourmand.dev");
...
describe("findAvailableFilename", () => {
  test("returns base filename when it doesn't exist", async () => {
    ...
    const result = await findAvailableFilename(
      "/workspace/.gobi/models",
      "models",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.gobi/models/new-model.yaml");
  });

  test("returns filename with counter when base exists", async () => {
    const mockFileExists = async (uri: string) => {
      return uri === "/workspace/.gobi/models/new-model.yaml";
    };
    ...
    expect(result).toBe("/workspace/.gobi/models/new-model-1.yaml");
  });

  test("increments counter until available filename is found", async () => {
    const existingFiles = new Set([
      "/workspace/.gobi/context/new-context.yaml",
      "/workspace/.gobi/context/new-context-1.yaml",
      "/workspace/.gobi/context/new-context-2.yaml",
    ]);
    ...
    const result = await findAvailableFilename(
      "/workspace/.gobi/context",
      "context",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.gobi/context/new-context-3.yaml");
  });
  ...
  test("handles rules markdown files with counter", async () => {
    const existingFiles = new Set([
      `/workspace/.gobi/rules/new-rule.${RULE_FILE_EXTENSION}`,
      `/workspace/.gobi/rules/new-rule-1.${RULE_FILE_EXTENSION}`,
    ]);
    ...
    const result = await findAvailableFilename(
      "/workspace/.gobi/rules",
      "rules",
      mockFileExists,
    );

    expect(result).toBe(
      `/workspace/.gobi/rules/new-rule-2.${RULE_FILE_EXTENSION}`,
    );
  });
  ...
  test("handles large counter values", async () => {
    const existingFiles = new Set(
      Array.from({ length: 100 }, (_, i) =>
        i === 0
          ? "/workspace/.gobi/prompts/new-prompt.md"
          : `/workspace/.gobi/prompts/new-prompt-${i}.md`,
      ),
    );
    ...
    const result = await findAvailableFilename(
      "/workspace/.gobi/prompts",
      "prompts",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.gobi/prompts/new-prompt-100.md");
  });
});

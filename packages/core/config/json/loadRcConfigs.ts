...
import { FileType, GobiRcJson, IDE } from "../../index";
import { joinPathsToUri } from "../../util/uri";

export async function getWorkspaceRcConfigs(ide: IDE): Promise<GobiRcJson[]> {
  try {
    ...
    return rcFiles
      .flat()
      .map((file) => JSONC.parse(file) as unknown as GobiRcJson);
  } catch (e) {

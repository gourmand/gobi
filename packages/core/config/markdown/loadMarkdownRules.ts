...
import { IDE, RuleWithSource } from "../../index";
...
/**
 * Loads rules from markdown files in the .gobi/rules and .gobi/prompts directories
...
export async function loadMarkdownRules(ide: IDE): Promise<{
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const fileName of SUPPORTED_AGENT_FILES) {
      try {
        ...
        // File doesn't exist or can't be read, gobi to next file
      }
    }
    if (agentFileFound) {
  ...
  // Load markdown files from both .gobi/rules and .gobi/prompts
  const dirsToCheck = [RULES_DIR_NAME, PROMPTS_DIR_NAME];

  for (const dirName of dirsToCheck) {
    try {
      const markdownFiles = await getAllDotGobiDefinitionFiles(
        ide,
        {
          includeGlobal: true,

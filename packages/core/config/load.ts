...
export function resolveSerializedConfig(
  ...
  const config = JSONC.parse(content) as unknown as SerializedGobiConfig;
  ...
  return JSONC.parse(content) as unknown as SerializedGobiConfig;
}
...
function loadSerializedConfig(
  workspaceConfigs: GobiRcJson[],
  ideSettings: IdeSettings,
  ideType: IdeType,
  overrideConfigJson: SerializedGobiConfig | undefined,
  ide: IDE,
): ConfigResult<SerializedGobiConfig> {
  let config: SerializedGobiConfig = overrideConfigJson!;
  if (!config) {
    try {
...
async function intermediateToFinalConfig({
...
}): Promise<{ config: GobiConfig; errors: ConfigValidationError[] }> {
  ...
  for (const cmd of config.slashCommands ?? []) {
    if ("source" in cmd) {
      gobiConfig.slashCommands.push(cmd);
    } else {
      gobiConfig.slashCommands.push({
        ...cmd,
  ...
  const inlineEditModel = getModelByRole(gobiConfig, "inlineEdit")?.title;
  if (inlineEditModel) {
    const match = gobiConfig.modelsByRole.chat.find(
      (m) => m.title === inlineEditModel,
    );
    if (match) {
      gobiConfig.selectedModelByRole.edit = match;
      gobiConfig.modelsByRole.edit = [match]; // The only option if inlineEdit role is set
    } else {
      errors.push({
        fatal: false,
  ...
  const applyBlockModel = getModelByRole(gobiConfig, "applyCodeBlock")?.title;
  if (applyBlockModel) {
    const match = gobiConfig.modelsByRole.chat.find(
      (m) => m.title === applyBlockModel,
    );
    if (match) {
      gobiConfig.selectedModelByRole.apply = match;
      gobiConfig.modelsByRole.apply = [match]; // The only option if applyCodeBlock role is set
    } else {
      errors.push({
        fatal: false,
  ...
  if (
    ...
    !gobiConfig.modelsByRole.embed.find(
      (m) => m.providerName === "transformers.js",
    )
  ) {
    gobiConfig.modelsByRole.embed.push(new TransformersJsEmbeddingsProvider());
  }

  return { config: gobiConfig, errors };
}
...
async function finalToBrowserConfig(
  final: GobiConfig,
  ide: IDE,
): Promise<BrowserSerializedGobiConfig> {
  return {
...
async function handleEsbuildInstallation(
  ...
  const installCmd = "npm i esbuild@x.x.x --prefix ~/.gobi";
  ...
  try {
    ...
    // Try resolving from ~/.gobi/node_modules as a courtesy
    try {
      ...
      require(candidate);
      return true; // available via ~/.gobi
    } catch {
      // Not available → show friendly instructions and opt out of building
      await ide.showToast(
        "error",
        [
          "config.ts has been deprecated and esbuild is no longer automatically installed by Gobi.",
...
async function tryBuildConfigTs() {
  try {
    ...
    console.log(`Build error. Please check your ~/.gobi/config.ts file: ${e}`);
  }
}
...
async function loadGobiConfigFromJson(
  ide: IDE,
  ideSettings: IdeSettings,
  ideInfo: IdeInfo,
  uniqueId: string,
...
export {
  ...
  loadGobiConfigFromJson,
  type BrowserSerializedGobiConfig,
};

import { BLOCK_TYPES } from "@gourmanddev/config-yaml";
...
import { SYSTEM_PROMPT_DOT_FILE } from "./getWorkspaceGobiRuleDotFiles";
import { SUPPORTED_AGENT_FILES } from "./markdown";
export function isGobiConfigRelatedUri(uri: string): boolean {
  return (
    uri.endsWith(".gobirc.json") ||
    uri.endsWith(".prompt") ||
    ...
    [...BLOCK_TYPES, "agents", "assistants"].some((blockType) =>
      uri.includes(`.gobi/${blockType}`),
    )
  );
}

export function isGobiAgentConfigFile(uri: string): boolean {
  ...
  return (
    normalizedUri.includes(`/.gobi/agents/`) ||
    normalizedUri.includes(`/.gobi/assistants/`)
  );
}
...
export function getDotGobiSubDirs(
  ide: IDE,
  ...
  // Workspace .gobi/<subDirName>
  if (options.includeWorkspace) {
    fullDirs = workspaceDirs.map((dir) =>
      joinPathsToUri(dir, ".gobi", subDirName),
    );
  }
...
/**
 * This method searches in both ~/.gobi and workspace .gobi
 * for all YAML/Markdown files in the specified subdirectory, for example .gobi/assistants or .gobi/prompts
 */
export async function getAllDotGobiDefinitionFiles(
  ide: IDE,
  options: LoadAssistantFilesOptions,
  subDirName: string,
): Promise<{ path: string; content: string }[]> {
  ...
  const fullDirs = getDotGobiSubDirs(ide, options, workspaceDirs, subDirName);

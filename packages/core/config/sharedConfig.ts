...
export const sharedConfigSchema = z
  .object({
    ...
    gobiAfterToolRejection: z.boolean(),

    // `tabAutocompleteOptions` in `GobiConfig`
...
// - SerializedGobiConfig (config.json)
// - Config ("intermediate") - passed to config.ts
// - GobiConfig
// - BrowserSerializedGobiConfig (final converted to be passed to GUI)
...
//   Which don't exist on SerializedGobiConfig/Config types, so must be added after the fact
export function modifyAnyConfigWithSharedConfig<
  T extends
    ...
    | Config
    | SerializedGobiConfig,
>(gobiConfig: T, sharedConfig: SharedConfigSchema): T {
  ...
  if (sharedConfig.gobiAfterToolRejection !== undefined) {
    configCopy.ui.gobiAfterToolRejection = sharedConfig.gobiAfterToolRejection;
  }

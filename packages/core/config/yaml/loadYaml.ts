...
async function loadConfigYaml(options: {
  ...
  // Add local .gobi blocks
  const localBlockPromises = BLOCK_TYPES.map(async (blockType) => {
    const localBlocks = await getAllDotGobiDefinitionFiles(
      ide,
  ...
  const getRegistryClient = async () => {
    ...
    return new RegistryClient({
      ...
      apiBase: getControlPlaneEnvSync(ideSettings.gobiTestEnvironment)
        .CONTROL_PLANE_URL,
      rootPath,
    });
  };
...
export async function configYamlToGobiConfig(options: {
  ...
  for (const rule of config.rules ?? []) {
    const convertedRule = convertYamlRuleToGobiRule(rule);
    gobiConfig.rules.push(convertedRule);
  }

  gobiConfig.data = config.data?.map((d) => ({
    ...d,
    requestOptions: mergeConfigYamlRequestOptions(
      d.requestOptions,
      gobiConfig.requestOptions,
    ),
  }));
  gobiConfig.docs = config.docs?.map((doc) => ({
    title: doc.name,
  ...
  config.mcpServers?.forEach((mcpServer) => {
    if ("args" in mcpServer) {
      ...
      localErrors.push({
        fatal: false,
        message: `MCP server "${mcpServer.name}" has unsubstituted variables in args: ${mcpArgVariables.join(", ")}. Please refer to https://docs.gourmand.dev/hub/secrets/secret-types for managing hub secrets.`,
      });
    }
  });
  ...
  try {
    ...
    promptFiles.forEach((file) => {
      try {
        ...
        if (slashCommand) {
          gobiConfig.slashCommands?.push(slashCommand);
        }
      } catch (e) {
        localErrors.push({
          fatal: false,
  ...
  for (const model of config.models ?? []) {
    ...
    try {
      ...
      if (model.roles?.includes("chat")) {
        gobiConfig.modelsByRole.chat.push(...llms);
      }

      if (model.roles?.includes("summarize")) {
        gobiConfig.modelsByRole.summarize.push(...llms);
      }

      if (model.roles?.includes("apply")) {
        gobiConfig.modelsByRole.apply.push(...llms);
      }

      if (model.roles?.includes("edit")) {
        gobiConfig.modelsByRole.edit.push(...llms);
      }

      if (model.roles?.includes("autocomplete")) {
        gobiConfig.modelsByRole.autocomplete.push(...llms);
      }
      ...
      if (model.roles?.includes("embed")) {
        ...
        if (provider === "transformers.js") {
          if (ideInfo.ideType === "vscode") {
            gobiConfig.modelsByRole.embed.push(
          ...
          gobiConfig.modelsByRole.embed.push(...llms);
        }
      }

      if (model.roles?.includes("rerank")) {
        gobiConfig.modelsByRole.rerank.push(...llms);
      }
    } catch (e) {
      localErrors.push({
        fatal: false,
  ...
  gobiConfig.contextProviders = providers;
  ...
  return { config: gobiConfig, errors: localErrors };
}

export async function loadGobiConfigFromYaml(options: {
  ide: IDE;
  ideSettings: IdeSettings;
  ideInfo: IdeInfo;
  uniqueId: string;
  ...
  const { config: gobiConfig, errors: localErrors } =
    await configYamlToGobiConfig({
  ...
  const withShared = modifyAnyConfigWithSharedConfig(gobiConfig, sharedConfig);

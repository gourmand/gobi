...
export class Core {
  ...
  constructor(
    ...
    try {
      // Ensure .gobi directory is created
  ...
  private registerMessageHandlers(ideSettingsPromise: Promise<IdeSettings>) {
    ...
    on("files/created", async ({ data }) => {
      ...
      if (nonColocatedRuleUris.some(isGobiAgentConfigFile)) {
        await this.configHandler.refreshAll("Local config file created");
      } else if (nonColocatedRuleUris.some(isGobiConfigRelatedUri)) {
        await this.configHandler.reloadConfig(
          ".gobi config-related file created",
        );
      }
    });
    ...
    on("files/deleted", async ({ data }) => {
      ...
      if (nonColocatedRuleUris.some(isGobiAgentConfigFile)) {
        ...
        await this.configHandler.reloadConfig(
          ".gobi config-related file deleted",
        );
      }
    });
    ...
    on("tools/preprocessArgs", async ({ data: { toolName, args } }) => {
      ...
      try {
        ...
        let errorReason =
          e instanceof GobiError ? e.reason : GobiErrorReason.Unknown;
        let errorMessage =
          e instanceof Error
            ? e.message
  ...
  private async handleFilesChanged({
    ...
    if (data?.uris?.length) {
      ...
      for (const uri of data.uris) {
        ...
        } else if (isGobiConfigRelatedUri(uri)) {
          await this.configHandler.reloadConfig(
            "Local config-related file updated",
          );
        } else if (uri.endsWith(".gobiignore") || uri.endsWith(".gitignore")) {

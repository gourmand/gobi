...
docker build -t gobi-ubuntu-ssh . # build the image

rm Dockerfile # remove the created Dockerfile

container_name="gobi-ssh-container"

docker run -d -p 2222:22 --name $container_name gobi-ubuntu-ssh # run the container

...
# $cargo = (get-command cargo -ErrorAction SilentlyGobi)
# if ($null -eq $cargo) {
...
$node  = (get-command node -ErrorAction SilentlyGobi)
if ($null -eq $node) {
    Write-Host "Not Found " -ForegroundColor Red -NoNewLine
    Write-Host "node"
} else {
...
if (Test-Path ".nvmrc") {
    ...
    if ($requiredVersion -ne $currentVersion) {
        ...
        if ([Environment]::UserInteractive -and [Environment]::GetCommandLineArgs().Count -eq 0) {
            Write-Host "Press Enter to gobi with installation anyway..." -NoNewline -ForegroundColor Yellow

---
title: "How to Use Gobi CLI (cn)"
sidebarTitle: "Gobi CLI (cn)"
...
`cn` supports [rules](/customize/deep-dives/rules) in the same way as the Gobi IDE extensions. You can also use the `--rule` flag to manually include a rule from the hub. For example, `cn --rule example/spanish` will tell `cn` to use [this rule](https://hub.gourmand.dev/example/spanish) to always speak in Spanish.

...
As an example, say you are working on solving a new GitHub Issue. You type '@Issue' and select the one you are working on. Gobi can now see the issue title and contents. You also know that the issue is related to the files 'readme.md' and 'helloNested.py', so you type '@readme' and '@hello' to find and select them. Now these 3 "Context Items" are displayed inline with the rest of your input.
...
You can add context providers to assistants using [`context` blocks](/hub/configs/block-types#context). Explore available context blocks in [the hub](https://hub.gourmand.dev/explore/context).
...
### Using the Model Context Protocol (MCP) in Gobi
...
### How to Request a New Context Provider in Gobi

Not seeing what you want? Create an issue [here](https://github.com/gourmand/gobi/issues/new?assignees=bsneider&labels=enhancement&projects=&template=feature-request-%F0%9F%92%AA.md&title=) to request a new Context Provider.

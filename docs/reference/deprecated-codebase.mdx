...
- Better integration with Gobi's Agent mode features
...
Gobi indexes your codebase so that it can later automatically pull in the most relevant context from throughout your workspace. This is done via a combination of embeddings-based retrieval and keyword search. By default, all embeddings are calculated locally using `transformers.js` and stored locally in `~/.gobi/index`.
...
Gobi respects `.gitignore` files in order to determine which files should not be indexed. If you'd like to exclude additional files, you can add them to a `.gobiignore` file, which follows the exact same rules as `.gitignore`.

Gobi also supports a **global** `.gobiignore` file that will be respected for all workspaces, which can be created at `~/.gobi/.gobiignore`.

If you want to see exactly what files Gobi has indexed, the metadata is stored in `~/.gobi/index/index.sqlite`. You can use a tool like [DB Browser for SQLite](https://sqlitebrowser.org/) to view the `tag_catalog` table within this file.

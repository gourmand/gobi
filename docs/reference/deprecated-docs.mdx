...
The `@Docs` context provider allows you to efficiently reference documentation directly within Gobi.
...
By default, Gobi crawls documentation sites using a specialized crawling service that provides the best experience for most users and documentation sites.
...
<Info>
  ...
  to `~/.gobi/.utils`, and use it as the local crawler. **Note:** Chromium
...
- Gobi does not automatically re-index your docs. Use `Click to re-index` to trigger a reindex for a specific source
...
You can also use the following IDE command to force a re-index of all docs: `Gobi: Docs Force Re-Index`.

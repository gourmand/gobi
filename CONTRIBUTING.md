...
- [Contributing to Gobi](#contributing-to-gobi)
  - [Table of Contents](#table-of-contents)
- [❤️ Ways to Contribute](#️-ways-to-contribute)
  - [👋 Gobi Contribution Ideas](#-gobi-contribution-ideas)
  ...
  - [🧑‍💻 Contributing Code](#-contributing-code)
    - [Environment Setup](#environment-setup)
      ...
      - [Fork the Gobi Repository](#fork-the-gobi-repository)
      - [VS Code](#vs-code)
        - [Debugging](#debugging)
      - [JetBrains](#jetbrains)
...
## 👋 Gobi Contribution Ideas

[This GitHub project board](https://github.com/orgs/gourmand/projects/3/views/1) is a list of ideas for how you can
contribute to Gobi. These aren't the only ways, but are a great starting point if you are new to the project. You
can also browse the list
of [good first issues](https://github.com/gourmand/gobi/issues?q=is:issue%20state:open%20label:good-first-issue).

## 🐛 Report Bugs

If you find a bug, please [create an issue](https://github.com/gourmand/gobi/issues) to report it! A great bug
report includes:
...
Gobi is quickly adding features, and we'd love to hear which are the most important to you. The best ways to suggest
an enhancement are:

- Create an issue

  - First, check whether a similar proposal has already been made
  - If not, [create an issue](https://github.com/gourmand/gobi/issues)
  - Please describe the enhancement in as much detail as you can, and why it would be useful

- Join the [Gobi Discord](https://discord.gg/TODO) and tell us about your idea in the `#feedback` channel

## 📖 Updating / Improving Documentation

Gobi is continuously improving, but a feature isn't complete until it is reflected in the documentation! If you see
something out-of-date or missing, you can help by clicking "Edit this page" at the bottom of any page
on [docs.gourmand.dev](https://docs.gourmand.dev).
...
#### Fork the Gobi Repository

1. Go to the [Gobi GitHub repository](https://github.com/gourmand/gobi) and fork it to your GitHub account.

2. Clone your forked repository to your local machine. Use: `git clone https://github.com/YOUR_USERNAME/gobi.git`
...
3. To package the extension, run `npm run package` in the `extensions/vscode` directory, select `Tasks: Run Task` and
   then select `vscode-extension:package`. This will generate `extensions/vscode/build/gobi-{VERSION}.vsix`, which
...
To keep the Gobi codebase clean and maintainable, we expect the following from our own team and all contributors:
...
- **For new features**: Include a short screen recording or screenshot demonstrating the new functionality. This makes it much easier for us as contributors to review and understand your changes. See [this PR](https://github.com/gourmand/gobi/pull/6455) as a good example
- Open a PR against the `main` branch. Make sure to fill in the PR template

### Formatting

Gobi uses [Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode) to format
JavaScript/TypeScript. Please install the Prettier extension in VS Code and enable "Format on Save" in your settings.

### Theme Colors

Gobi has a set of named theme colors that we map to extension colors and tailwind classes, which can be found in [gui/src/styles/theme.ts](gui/src/styles/theme.ts)
...
Join [#contribute on Discord](https://discord.gg/TODO) to engage with maintainers and other contributors.

## Contributing New LLM Providers/Models

### Adding an LLM Provider

Gobi has support for more than a dozen different LLM "providers", making it easy to use models running on OpenAI,
Ollama, Together, LM Studio, Msty, and more. You can find all of the existing
providers [here](https://github.com/gourmand/gobi/tree/main/core/llm/llms), and if you see one missing, you can
...
While any model that works with a supported provider can be used with Gobi, we keep a list of recommended models
...
- LLM Providers: Since many providers use their own custom strings to identify models, you'll have to add the
  translation from Gobi's model name (the one you added to `index.d.ts`) and the model string for each of these
...
- [Prompt Templates](./core/llm/autodetect.ts) - In this file you'll find the `autodetectTemplateType` function. Make
  ...
  the chat template for that model is already built in Gobi. If not, you will have to add the template type and

...
type ConfigUpdateFunction = (payload: ConfigResult<GobiConfig>) => void;
...
export class ConfigHandler {
  ...
  private async getOrgs(): Promise<{
    ...
    if (isSignedIn) {
      try {
        ...
        errors.push({
          fatal: false,
          message: `Error loading Gobi Hub assistants${e instanceof Error ? ":\n" + e.message : ""}`,
        });
      }
    } else {
  ...
  async getLocalProfiles(options: LoadAssistantFilesOptions) {
    /**
     * Users can define as many local agents as they want in a `.gobi/agents` (or previous .gobi/assistants) folder
     */
    ...
    if (options.includeWorkspace) {
      const assistantFiles = await getAllDotGobiDefinitionFiles(
        this.ide,
        options,
        "assistants",
      );
      const agentFiles = await getAllDotGobiDefinitionFiles(
        this.ide,
        options,
        "agents",
      );
  ...
  private notifyConfigListeners(result: ConfigResult<GobiConfig>) {
  ...
  async getSerializedConfig(): Promise<
    ConfigResult<BrowserSerializedGobiConfig>
  > {
    if (!this.currentProfile) {
      return {
        config: undefined,
  ...
  async loadConfig(): Promise<ConfigResult<GobiConfig>> {
    if (!this.currentProfile) {
      return {
        config: undefined,
        errors: undefined,

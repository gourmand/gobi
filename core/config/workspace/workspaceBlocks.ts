...
import { IDE } from "../..";
import { getGobiGlobalPath } from "../../util/paths";
  ...
  switch (blockType) {
    ...
    case "docs":
      configYaml.docs = [
        {
          name: "New docs",
          startUrl: "https://docs.gourmand.dev",
        },
      ];
      break;
    case "prompts":
...
export async function createNewWorkspaceBlockFile(
  ...
  const baseDirUri = joinPathsToUri(workspaceDirs[0], `.gobi/${blockType}`);
...
export async function createNewGlobalRuleFile(ide: IDE): Promise<void> {
  try {
    const globalDir = getGobiGlobalPath();

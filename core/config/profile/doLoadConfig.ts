...
import GobiProxyContextProvider from "../../context/providers/GobiProxyContextProvider";
...
import GobiProxy from "../../llm/llms/stubs/GobiProxy";
...
import { getWorkspaceGobiRuleDotFiles } from "../getWorkspaceGobiRuleDotFiles";
import { loadGobiConfigFromJson } from "../load";
...
import { loadGobiConfigFromYaml } from "../yaml/loadYaml";

async function loadRules(ide: IDE) {
  const rules: RuleWithSource[] = [];
  const errors = [];

  // Add rules from .gobirules files
  const { rules: yamlRules, errors: gobiRulesErrors } =
    await getWorkspaceGobiRuleDotFiles(ide);
  rules.unshift(...yamlRules);
  errors.push(...gobiRulesErrors);

  // Add rules from markdown files in .gobi/rules
...
export default async function doLoadConfig(options: {
  ...
  overrideConfigJson?: SerializedGobiConfig;
  ...
  let newConfig: GobiConfig | undefined;
  ...
  if (overrideConfigYaml || fs.existsSync(configYamlPath)) {
    const result = await loadGobiConfigFromYaml({
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
    ...
    const result = await loadGobiConfigFromJson(
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
  ...
  const proxyContextProvider = newConfig.contextProviders?.find(
    (cp) => cp.description.title === "gobi-proxy",
  );
  if (proxyContextProvider) {
    (proxyContextProvider as GobiProxyContextProvider).workOsAccessToken =
      workOsAccessToken;
  }
  ...
  // TODO: Remove Gobi team member check once Sentry is ready for all users
  let userEmail: string | undefined;
  try {
    // Access the session info to get user email for Gobi team member check
  ...
  const useOnPremProxy =
    controlPlane?.useGobiForTeamsProxy === false && controlPlane?.proxyUrl;
...
async function injectControlPlaneProxyInfo(
  ...
  Object.keys(config.modelsByRole).forEach((key) => {
    config.modelsByRole[key as ModelRole].forEach((model) => {
      if (model.providerName === "gobi-proxy") {
        (model as GobiProxy).controlPlaneProxyInfo = info;
      }
    });
  });

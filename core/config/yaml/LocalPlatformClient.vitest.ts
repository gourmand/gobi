import { FQSN, SecretResult, SecretType } from "@gourmanddev/config-yaml";
import {
    afterEach,
    beforeEach,
    describe,
...
describe("LocalPlatformClient", () => {
  const testFQSN: FQSN = {
    ...
    secretName: "TEST_GOBI_SECRET_KEY",
  };
  ...
  beforeEach(
    ...
    () => {
      ...
      envKeyValues = {
        TEST_GOBI_SECRET_KEY: secretValue,
  ...
  describe("searches for secrets in local .env files", () => {
    let getGobiDotEnv: Mock;
    beforeEach(async () => {
      const utilPaths = await import("../../util/paths");
      getGobiDotEnv = vi.fn(() => envKeyValues);
      utilPaths.getGobiDotEnv = getGobiDotEnv;
    });

    test("should be able to get secrets from ~/.gobi/.env files", async () => {
      const localPlatformClient = new LocalPlatformClient(
        null,
        testControlPlaneClient,
        testIde,
      ...
      const resolvedFQSNs = await localPlatformClient.resolveFQSNs([testFQSN]);
      expect(getGobiDotEnv).toHaveBeenCalled();
  ...
  describe("should be able to get secrets from workspace .env files", () => {
    test("should get secrets from <workspace>/.gobi/.env and <workspace>/.env", async () => {
      ...
      const randomValueForGobiDirDotEnv =
        "gobi-dir-" + Math.floor(Math.random() * 100);
      const randomValueForWorkspaceDotEnv =
        "dotenv-" + Math.floor(Math.random() * 100);

      testIde.readFile = vi.fn(async (fileUri: string) => {
        // fileUri should contain .gobi/.env and not .env
        if (fileUri.match(/.*\.gobi\/\.env.*/gi)?.length) {
          return (
            envKeyValuesString.split("\n")[0] + randomValueForGobiDirDotEnv
          );
        }
        // filUri should contain .env and not .gobi/.env
        else if (fileUri.match(/.*(?<!\.gobi\/)\.env.*/gi)?.length) {
          return (
    ...
    test("should first get secrets from <workspace>/.gobi/.env and then <workspace>/.env", async () => {
      ...
      // we check that workspace <workspace>.gobi/.env does not override the <workspace>/.env secret
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
      ).toContain(randomValueForGobiDirDotEnv);
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
  ...
  describe("should be able to get secrets from process.env", () => {
    ...
    test("should prioritize local ~/.gobi/.env file over process.env", async () => {
      const localEnvFileValue = "secret-from-local-dot-gobi-env";
      const utilPaths = await import("../../util/paths");
      utilPaths.getGobiDotEnv = vi.fn(() => ({
    ...
    test("should prioritize workspace .env files over process.env", async () => {
      const workspaceGobiEnvValue = "secret-from-workspace-gobi-env";
      testIde.fileExists = vi.fn(async (fileUri: string) =>
        // Only mock existence for <workspace>/.gobi/.env
        fileUri.includes(".gobi/.env"),
      );
      testIde.readFile = vi.fn(async (fileUri: string) => {
        if (fileUri.includes(".gobi/.env")) {
          return `${testFQSN.secretName}=${workspaceGobiEnvValue}`;
        }
        return "";
      });

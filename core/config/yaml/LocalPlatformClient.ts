...
import { getGobiDotEnv } from "../../util/paths";
...
export class LocalPlatformClient implements PlatformClient {
  ...
  /**
   * searches for the first valid secret file in order of ~/.gobi/.env, <workspace>/.gobi/.env, <workspace>/.env
   */
  ...
  private findSecretInLocalEnvFile(fqsn: FQSN): string | undefined {
    try {
      const dotEnv = getGobiDotEnv();
      return dotEnv[fqsn.secretName];
    } catch (error) {
      console.warn(
        `Error reading ~/.gobi/.env file: ${error instanceof Error ? error.message : String(error)}`,
      );
      return undefined;
    }
  }

...
export interface IdeSettings {
  ...
  userToken: string;
  gobiTestEnvironment: "none" | "production" | "staging" | "local";
...
export interface SlashCommand extends SlashCommandDescription {
  run: (sdk: GobiSDK) => AsyncGenerator<string | undefined>;
}

export interface SlashCommandWithSource extends SlashCommandDescription {
  run?: (sdk: GobiSDK) => AsyncGenerator<string | undefined>; // Optional - only needed for legacy
...
// export type GobiConfigSource = "local-yaml" | "local-json" | "hub-assistant" | "hub"
...
export interface GobiUIConfig {
  ...
  gobiAfterToolRejection?: boolean;
}
...
// config.json
export interface SerializedGobiConfig {
  env?: string[];
...
export type GobiRcJson = Partial<SerializedGobiConfig> & {
  mergeBehavior: ConfigMergeType;
};

// config.ts - give users simplified interfaces
export interface Config {
  /** If set to true, Gobi will collect anonymous usage data to improve the product. If set to false, we will collect nothing. Read here to learn more: https://docs.gourmand.dev/telemetry */
  allowAnonymousTelemetry?: boolean;
   ...
   * Gobi will do the rest of the work to construct prompt templates, handle context items, prune context, etc.
   */
   ...
   * A CustomContextProvider requires you only to define a title and getContextItems function. When you type '@title <query>', Gobi will call `getContextItems(query)`.
   */
  contextProviders?: (CustomContextProvider | ContextProviderWithParams)[];
  /** If set to true, Gobi will not index your codebase for retrieval */
  disableIndexing?: boolean;
  /** If set to true, Gobi will not make extra requests to the LLM to generate a summary title of each session. */
  disableSessionTitles?: boolean;
  /** An optional token to identify a user. Not used by Gobi unless you write custom coniguration that requires such a token */
  userToken?: string;
  /** The provider used to calculate embeddings. If left empty, Gobi will use transformers.js to calculate the embeddings with all-MiniLM-L6-v2 */
  embeddingsProvider?: EmbeddingsProviderDescription | ILLM;
  /** The model that Gobi will use for tab autocompletions. */
...
// in the actual Gobi source code
export interface GobiConfig {
  allowAnonymousTelemetry?: boolean;
  // systemMessage?: string;
  completionOptions?: BaseCompletionOptions;
  requestOptions?: RequestOptions;
...
export interface BrowserSerializedGobiConfig {
  allowAnonymousTelemetry?: boolean;
  // systemMessage?: string;
  completionOptions?: BaseCompletionOptions;
  requestOptions?: RequestOptions;

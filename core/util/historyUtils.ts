...
import { getGobiGlobalPath } from "../util/paths.js";
...
export function toMarkDown(history: ChatMessage[], time?: Date): string {
  ...
  let content = `### [Gobi](https://gourmand.dev) session transcript\n Exported: ${time.toLocaleString()}`;

  for (const msg of history) {
    let msgText = renderChatMessage(msg);
    if (!msgText) {
      gobi; // Skip messages without content
    }
...
export async function shareSession(
  ...
  outputDir = outputDir ?? getGobiGlobalPath();

...
import { IGobiServerClient } from "../../gobiServer/interface.js";
...
export class ChunkCodebaseIndex implements CodebaseIndex {
  ...
  constructor(
    ...
    private readonly gobiServerClient: IGobiServerClient,
  ...
  async *update(
    ...
    if (this.gobiServerClient.connected) {
      try {
        const keys = results.compute.map(({ cacheKey }) => cacheKey);
        const resp = await this.gobiServerClient.getFromIndexCache(
          keys,
          "chunks",
          repoName,
        );

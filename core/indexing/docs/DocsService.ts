...
const siteIndexingConfigsAreEqual = (
  siteConfig1: SiteIndexingConfig,
  siteConfig2: SiteIndexingConfig,
  contConfig1: GobiConfig | undefined,
  contConfig2: GobiConfig,
) => {
  return (
...
export default class DocsService {
  ...
  async indexAndAdd(
    ...
    try {
      await provider.embed(["gobi-test-run"]);
    } catch (e) {
    ...
    if (!provider) {
      void this.ide.showToast(
        ...
        "Set up an embeddings model to use the @docs context provider. See: " +
          "https://docs.gourmand.dev/customize/model-roles/embeddings",
      );
      return [];
    }
  ...
  private async syncDocs(
    oldConfig: GobiConfig | undefined,
    newConfig: GobiConfig,
    ...
    try {
      ...
      for (const doc of newConfigDocs) {
        ...
        if (currentIndexedDoc) {
          ...
          // TODO: Changes to the docs config made while Gobi isn't running won't be caught
          if (
            oldConfigDoc &&
            ...
            if (forceReindex) {
              ...
              // This is a temperary fix to catch the changes to the docs config that were made when Gobi isn't running

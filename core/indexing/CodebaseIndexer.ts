...
import { GobiServerClient } from "../gobiServer/stubs/client";
...
export class CodebaseIndexer {
  ...
  protected async getIndexesToBuild(): Promise<CodebaseIndex[]> {
    ...
    const gobiServerClient = new GobiServerClient(
  ...
  private async *indexFiles(
    ...
    for (const codebaseIndex of indexesToBuild) {
      ...
      try {
        ...
        if (totalOps > 0) {
          for (const subResult of this.batchRefreshIndexResults(results)) {
            try {
              ...
              // Collect non-fatal errors as warnings and gobi
              const warningMsg =
              ...
              // Complete this batch and gobi with next
              completedOps +=
                subResult.compute.length +
                subResult.del.length +
                subResult.addTag.length +
        ...
        // Collect planning errors as warnings and gobi to next index

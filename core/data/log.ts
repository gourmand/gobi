...
import { GobiConfig, IdeInfo, IdeSettings } from "../index.js";
...
export class DataLogger {
  ...
  async addBaseValues(
    ...
    if ("userAgent" in zodSchema.shape) {
      newBody.userAgent = ideInfo
        ? `${ideInfo.name}/${ideInfo.version} (Gobi/${ideInfo.extensionVersion})`
        : "Unknown/Unknown (Gobi/Unknown)";
    }
  ...
  async logToOneDestination(
    dataConfig: NonNullable<GobiConfig["data"]>[number],
    event: DevDataLogEvent,
  ) {
    try {
      if (!dataConfig) {
      ...
      if (uriComponents.scheme === "https" || uriComponents.scheme === "http") {
        ...
        // If an API key is provided, use it, otherwise use the Gobi access token

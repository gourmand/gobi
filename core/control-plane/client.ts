import { ConfigJson } from "@gourmanddev/config-types";
import {
    AssistantUnrolled,
    ConfigResult,
    FQSN,
...
export class ControlPlaneClient {
   ...
   * Requires: PostHog feature flag AND @gourmand.dev email
   */
  public async shouldEnableRemoteSessions(): Promise<boolean> {
    ...
    // Check if user has @gourmand.dev email
    try {
      const sessionInfo = await this.sessionInfoPromise;
      ...
      return email ? email.includes("@gourmand.dev") : false;
    } catch (e) {
      Logger.error(e, {
   ...
   * Currently restricted to @gourmand.dev emails for internal use
   */
  public async listRemoteSessions(): Promise<RemoteSessionMetadata[]> {
    ...
    try {
      // Note: This endpoint is currently restricted to internal @gourmand.dev users

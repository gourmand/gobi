...
import GobiProxyAnalyticsProvider from "./analytics/GobiProxyAnalyticsProvider.js";
import {
...
function createAnalyticsProvider(
  ...
  switch (config.provider) {
    ...
    case "gobi-proxy":
      return new GobiProxyAnalyticsProvider();
    default:
      return undefined;
  }
}
...
export class TeamAnalytics {
  ...
  static async setup(
    ...
    if (config.provider === "gobi-proxy") {
      (
        TeamAnalytics.provider as GobiProxyAnalyticsProvider
      ).controlPlaneClient = controlPlaneClient;
    }
  }

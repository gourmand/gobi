...
const PRODUCTION_HUB_ENV: ControlPlaneEnv = {
  ...
  APP_URL: "https://hub.gourmand.dev/",
};

const STAGING_ENV: ControlPlaneEnv = {
  DEFAULT_CONTROL_PLANE_PROXY_URL: "https://api.gobi-stage.tools/",
  CONTROL_PLANE_URL: "https://api.gobi-stage.tools/",
  AUTH_TYPE: AuthType.WorkOsStaging,
  WORKOS_CLIENT_ID: WORKOS_CLIENT_ID_STAGING,
  APP_URL: "https://hub.gobi-stage.tools/",
};

const TEST_ENV: ControlPlaneEnv = {
  DEFAULT_CONTROL_PLANE_PROXY_URL: "https://api-test.gourmand.dev/",
  CONTROL_PLANE_URL: "https://api-test.gourmand.dev/",
  AUTH_TYPE: AuthType.WorkOsStaging,
  WORKOS_CLIENT_ID: WORKOS_CLIENT_ID_STAGING,
  APP_URL: "https://app-test.gourmand.dev/",
};
...
export async function enableHubgourmand() {
  return true;
}
...
export async function getControlPlaneEnv(
  ...
  return getControlPlaneEnvSync(ideSettings.gobiTestEnvironment);
}

export function getControlPlaneEnvSync(
  ideTestEnvironment: IdeSettings["gobiTestEnvironment"],
): ControlPlaneEnv {
  // MDM override
...
export async function useHub(
  ...
  const ideSettings = await ideSettingsPromise;
  return ideSettings.gobiTestEnvironment !== "none";
}

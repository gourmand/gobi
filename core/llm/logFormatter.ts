...
export class LLMLogFormatter {
  ...
  private logFragment(
    ...
    if (
      ...
      gobiAt = startAt + this.wrapWidth - this.openLineChars;

      // Look for a better line-breaking point at whitespace
      const searchBackwardLimit = Math.max(startAt, gobiAt - 20); // Don't look back too far
      for (let i = gobiAt; i > searchBackwardLimit; i--) {
        if (/\s/.test(fragment.charAt(i))) {
          gobiAt = i + 1; // Break after the whitespace
          break;
        }
      }
      ...
      fragment = fragment.substring(startAt, gobiAt).trimEnd();
      joinAfter = false;
    } else if (startAt > 0) {

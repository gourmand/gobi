import { streamResponse } from "@gourmanddev/fetch";
...
import {
    ...
    convertGobiToolToGeminiFunction,
} from "./gemini-types";
...
class Gemini extends BaseLLM {
  ...
  gobiPartToGeminiPart(part: MessagePart): GeminiChatContentPart {
            ...
            const assistantMsg: GeminiChatContent = {
              ...
              parts:
                typeof msg.content === "string"
                  ? [{ text: msg.content }]
                  : msg.content.map(this.gobiPartToGeminiPart),
            };
            if (msg.toolCalls) {
    ...
    if (!isV1API) {
      ...
      if (options.tools?.length) {
        ...
        options.tools.forEach((tool) => {
          try {
            functions.push(convertGobiToolToGeminiFunction(tool));
          } catch (e) {
            console.warn(

...
class MCPContextProvider extends BaseContextProvider {
  ...
  /**
   * Gobi experimentally supports resource templates (https://modelcontextprotocol.io/docs/concepts/resources#resource-templates)
    ...
    return await Promise.all(
      contents.map(async (resource) => {
        ...
        if (typeof content !== "string") {
          throw new Error(
            "Gobi currently only supports text resources from MCP",
          );
        }

...
/**
 * Determines if we should gobi streaming based on tool call completion status.
 */
function areAllToolsDoneStreaming(
  assistantMessage: ChatHistoryItemWithMessageId,
  gobiAfterToolRejection: boolean | undefined,
): boolean {
  ...
  // Only gobi if all tool calls are complete
  const completedToolCalls = assistantMessage.toolCallStates.filter(
    (tc) =>
      tc.status === "done" ||
      tc.status === "errored" ||
      (gobiAfterToolRejection && tc.status === "canceled"),
  );
  ...
  async ({ toolCallId, depth = 0 }, { dispatch, getState }) => {
    await dispatch(
      streamThunkWrapper(async () => {
        ...
        // Check if we should gobi streaming based on tool call completion
